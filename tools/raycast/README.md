...
`backup` without an argument searches Raycast for the `Export Settings & Data`
command, uses AppleScript to press Return through the export/save dialogs, then
waits for a new `Raycast*.rayconfig` export in `~/Downloads` and copies it into
...
Raycast requires an export passphrase before it opens the directory picker.
Provide it without storing secrets in this repo:
